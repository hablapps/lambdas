...
class ParseTerm2Spec extends FunSpec with Matchers with Inside {

  it("Int literals") {
    inside(ParseTerm2[ShowB].apply(tr_int(1))[Unit, Unit].apply(())) {
  ...
  it("Add expressions") {
    inside(ParseTerm2[ShowB].apply(tr_add(tr_int(1), tr_int(2)))[Unit, Unit].apply(())) {
      case Right(DynLTerm(_, term)) =>
        term(0) shouldBe "(1+2)"
    }
  }

...
trait TreeSer[T, L <: Lambda[TreeSer.Show, T]] {
  def apply(l: L): TreeSer.Show[T]
}
...
object TreeSer {
  ...
  def apply[T, L <: Lambda[Show, T]](l: L)(implicit T: TreeSer[T, L]): Show[T] =
    T(l)
  ...
  // trait TreeSerConversion[T1, T2, L2[L1 <: Lambda[Show, T1]] <: Lambda[Show, T2]]{
  //   def apply[L1 <: Lambda[Show, T1]]: TreeSer[T2, L2[L1]]
  // }
